...
export async function createApplication(data: {
  ...
  try {
    const contentType = Stack.ContentType('application') as any;
    const entry = await contentType.Entry().create({
      entry: data,
    });
    return entry;
  } catch (error) {
...
export async function incrementApplicationUpvotes(applicationUid: string) {
  try {
    ...
    const updatedApp = await (Stack.ContentType('application') as any)
...
export async function addUpvotedApplication(userUid: string, applicationUid: string) {
  try {
    ...
    const updatedUser = await (Stack.ContentType('users') as any)
      .Entry(userUid)
      .update({
        entry: {
...
export async function publishEntry(params: {
  ...
  try {
    ...
    const publishedEntry = await (Stack.ContentType(contentTypeUid) as any)
      .Entry(entryUid)
      .publish({
        publishDetails: {
          environments,
